...
def blah(satListConflictGroups):
			...
			for time in blackList:
				...
				#sat conflicts with one of the times in blackList
					...
					if endGap>=transactionTime:
						...
						conflicts=True
						break #removing this causes a problem that be maybe indicative of a large problem
				else:
					thisSatPassStart = sat.AOS
					thisSatPassEnd = sat.AOS + transactionTime
					conflicts=False
	...
	#print(blackList)
		unScheduledSats = []
		unScheduledSats = [sat for sat in group if sat not in scheduledSats]
